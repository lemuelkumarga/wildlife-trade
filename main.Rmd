...
```{r}
pre_remove <- nrow(dataset)
post_remove_str <- function () {
  post_remove <- nrow(dataset)
  cat(sprintf("%i rows removed (%i%% of total)",pre_remove - post_remove,floor((pre_remove - post_remove)/pre_remove * 100)))  
}
...
The tricky part comes in when we convert a quantity from a non-target unit to the target unit. During this step, we will assume that, <span class="hl">for each species, the median quantity traded in animal units is constant regardless of the terms/units they were specified in</span>. In other words, if the median quantity of elephant bodies is 2, and the median quantity of elephant bodies in kgs is 14,000, then the 2 elephant bodies are equivalent to 14,000 kgs.
...
To sum up the flowchart above, we will first count the number of records under the given species, term and unit. If the records are insufficient, we then "roll-up" and find the number of records for the genus associated with the species. This process continues until we have sufficient data points to calculate the median.
...
Let us now <span class="hl">convert each term to the universal animal unit</span>. Through observation, the terms can be differentiated into well-defined or ambiguous. A term is well-defined if it each animal (which has them) has the same number of them. One good example would be tusk, since 2 of them are always the equivalent of 1 animal. 
...
The PageRank algorithm allows us to determine who the major players are. However, each country has different roles to play in endangered wildlife trading. Some can be <span class="hl">consumers</span> (countries which fuel the demand of wildlife consumption), <span class="hl">suppliers</span> (countries which capture the wildlife and ship them out) or even <span class="hl">dealers</span> (intermediaries which connect suppliers and consumers). To reduce wildlife trading, each role will require different strategies. <span class="hl">In this section, we will construct a method to identify the top consumers, dealers and suppliers.</span> Moreover, it is important to note that these roles are not exclusive; a country may well be a supplier of one species and a consumer of another.

To determine the country's degree of role as a consumer, dealer and supplier, we will first split the quantity traded of each species into three constituents:
...
These constitutients are then summed up across species to determine a country's net imports, net exports and transits. These 3 values are subsequently to decompose PageRank via the following approach.
...
* <span class="hl red-text">ConsumerRank</span> is the % of universal trades the country is responsible for due to <span class="hl red-text">its consumption</span>,
* <span class="hl">DealerRank</span> is the % of universal trades the country is responsible for due to <span class="hl">it being an intermediary</span>, and
* <span class="hl purple-text">SupplierRank</span> is the % of universal trades the country is responsible for due to <span class="hl purple-text">its provision<span>.

By ranking each of these three ranks, we can now determine which countries are the top consumers, dealers and suppliers. By summing up each of these metrics, we are also able to determine the major drivers of wildlife trading:
...
From the above summary, we know that consumption and supply of wildlife goods are the major factors behind wildlife exchanges. Intermediaries surprisingly do not play major roles. This suggests that in order to reduce trading, we are better off focusing on reducing supply and demand (instead of breaking down the channels connecting them).
...
The leaf plot above shows the degree of role for the top 25 major players. <span class="hl blue-text">United States</span>, the biggest player, are mostly consumers, while <span class="hl red-text">Germany</span> is a large dealer. <span class="hl orange-text">China</span> unsurprisingly is both consuming and supplying wildlife resources.
...
To find out more about each country's trading activity, please hover or click on the buttons below:
...
`r get_players((dealers %>% head(5))$index,heading_color, type="Transits", asterisks=list("FR"=c("European Eel"="Due to missing black market exports, France is considered an intermediary stop for European Eels, even though they originated from the country. See Limitations.")))`

#### Suppliers

`r get_players((suppliers %>% head(5))$index,get_color("purple"), type="Net_Exports", asterisks=list("MX"=c("European Eel"="Due to missing black market imports, European Eels are considered as Top Providers for Mexico. See Limitations."), "CN"=c("European Eel"="Due to missing black market imports, European Eels are considered as Top Providers for China. See Limitations.")))`
...
* <span class="hl">Consumers</span> are countries which generate the demand of wildlife. It is important to educate citizens in these countries to stop using wildlife-related products. The top 5 consumers are `r gsub(", Republic of", "", paste0(sprintf('<span class="hl">%s</span>',(consumers %>% head(5))$NAME), collapse = ", "))`.
...
* <span class="hl">Suppliers</span> are countries that provide the wildlife goods. Efforts to protect the nature reserves of these countries should be emphasized. The top 5 suppliers are `r paste0(sprintf('<span class="hl">%s</span>',(suppliers %>% head(5))$NAME), collapse = ", ")`.

## Limitations

The above analysis, though simplified, provide us with a better picture of the wildlife situation across the world. Listed below are some suggestions to improve the accuracy of results:

* <span class="hl">One Side of the Story</span>: This study is based on data reported by governments. However, there are many black market trades that could influence our understanding of the wildlife market. It would be interesting to identify the black market trades through verifying the consistency of the dataset, and then incorporating that into this analysis.
* <span class="hl">Wild</span>: Due to size constraints, we only considered trades whose sources are tagged as "Wild". By expanding the scope, additional insights could possibly be unearthed.
* <span class="hl">Species-Specific Fixes</span>: The methods in pre-processing are one-size-fits-all, which favor expediency over accuracy. By fixing each trade according to individual species's characteristics (such as weight), we can obtain more accurate transformations from the terms specified into animal counts.

## References

CITES Database 2018. <https://www.cites.org/>. Downloaded on 24 April 2018.
